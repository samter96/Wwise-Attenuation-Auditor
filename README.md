...
### ⚠️ 큰 프로젝트에서 결과가 누락될 수 있음

**Wwise 2025.1.x 이상에서 `ak.wwise.core.object.get { ofType: ["Sound"] }` 가
전체 Sound 가 아닌 일부(예: 8 개) 만 반환하는 케이스가 확인되었습니다.**
스캔 결과가 명백히 적게 나오면 WAAPI 응답이 잘리고 있을 가능성이 큽니다.

- 임시 회피: 스코프 트리에서 워크유닛 단위로 나눠 스캔
- 정식 해결은 추후 버전에서 hierarchy 순회 기반 수집 방식으로 변경 예정

### 그 외

- **WAAPI 가 비활성화** 되어 있으면 연결 실패. Wwise 의 User Preferences 에서 활성화 후 재연결.
- **Wwise 가 모달 다이얼로그를 띄운 상태** 면 WAAPI 호출이 차단됩니다.
- Wwise 종료 시 툴이 자동 종료되므로 결과가 필요하면 **CSV 로 먼저 내보낸 뒤** 종료하세요.
- `att_auditor_exceptions.json` 은 프로젝트별이 아니라 **툴 폴더 단위** 로 저장됩니다.
  여러 프로젝트를 오가며 사용한다면 별도 백업 / 분기 관리가 필요합니다.
...
| 결과가 비정상적으로 적음 (수십 개 미만) | 위 ⚠️ 항목 참조 |
